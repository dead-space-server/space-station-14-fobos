...
using Content.Shared.CrewManifest; // DS14
    ...
    public sealed partial class PaperWindow : BaseWindow
    {
        ...
        private void ToggleInsertDataPanel()
        {
            if (_insertDataWindow == null)
                BuildInsertDataWindow();

            if (_insertDataWindow!.IsOpen)
            {
        ...
        private void BuildInsertDataWindow()
        {
            _insertDataWindow = new DefaultWindow
            {
                Title = Loc.GetString("paper-ui-toolbar-insert"),
                SetSize = new Vector2(280, 320),
            };

            var root = new BoxContainer { Orientation = BoxContainer.LayoutOrientation.Vertical, Margin = new Thickness(4) };

            _insertDataStatusLabel = new Label { Text = Loc.GetString("paper-ui-insert-loading") };
            root.AddChild(_insertDataStatusLabel);
            ...
            root.AddChild(_insertDataManifestList);

            _insertDataWindow.Contents.AddChild(root);
        }

        /// <summary>
        /// Called by <c>PaperBoundUserInterface</c> when the server answers a data request.
        /// </summary>
        public void PopulateInsertData(PaperInsertDataResponseMessage data)
        {
            ...
            _insertDataStatusLabel!.Visible = false;
            _insertDataButtons!.Visible = true;
            _insertDataButtons.RemoveAllChildren();

            AddInsertDataButton(Loc.GetString("paper-ui-insert-station", ("station", data.StationName ?? Loc.GetString("paper-ui-insert-unknown"))), data.StationName);
            AddInsertDataButton(Loc.GetString("paper-ui-insert-datetime", ("datetime", data.RoundDateTime)), data.RoundDateTime);
            AddInsertDataButton(Loc.GetString("paper-ui-insert-name", ("name", data.CharacterName)), data.CharacterName);
        ...
        private void AddInsertDataButton(string label, string? value)
        {
            if (string.IsNullOrWhiteSpace(value))
                return;

            var button = new Button { Text = label, HorizontalExpand = true, ClipText = true };
            button.OnPressed += _ => InsertValue(value);
            _insertDataButtons!.AddChild(button);
        }

        /// <summary>
        /// Inserts a value at the cursor, replacing the current selection if there is one - same
        /// behaviour as the inline formatting buttons.
        /// </summary>
        private void InsertValue(string value)
        {
            ...
            var lower = Input.SelectionLower.Index;
            var upper = Input.SelectionUpper.Index;
            var newText = text[..lower] + value + text[upper..];

            CommitTextChange(newText, lower + value.Length);
        }
