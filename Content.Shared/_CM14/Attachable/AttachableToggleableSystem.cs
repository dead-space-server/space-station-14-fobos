...
public sealed class AttachableToggleableSystem : EntitySystem
{
    ...
    private void OnAttachableAltered(Entity<AttachableToggleableComponent> attachable, ref AttachableAlteredEvent args)
    {
        ...
        _actionsSystem.SetEnabled(attachable.Comp.Action, attachable.Comp.Attached);
    }
    ...
    {
        var exists = ent.Comp.Action != null && Exists(ent.Comp.Action.Value);
        if (!_actionContainerSystem.EnsureAction(ent, ref ent.Comp.Action, ent.Comp.ActionId))
        {
            if (ent.Comp.Action is { } existingAction && Exists(existingAction))
                _actionsSystem.GrantContainedAction(args.User, ent.Owner, existingAction);
            return;
        }
        ...
        {
            _actionsSystem.SetIcon(action.AsNullable(), ent.Comp.Icon);
            _actionsSystem.SetIconOn(action.AsNullable(), ent.Comp.IconActive);
            _actionsSystem.SetEnabled(action.AsNullable(), ent.Comp.Attached);
        }
    }
    ...
    private void OnGotEquippedHand(Entity<AttachableToggleableComponent> ent, ref GotEquippedHandEvent args)
    {
    ...
    private void OnGotUnequippedHand(Entity<AttachableToggleableComponent> ent, ref GotUnequippedHandEvent args)
    {
        if (ent.Comp.Action is not { } action)
            return;
        _actionsSystem.RemoveProvidedAction(args.User, ent, action);
