    ...
    private void AddLimb(Entity<HumanoidAppearanceComponent> body, string slot, Entity<BodyPartComponent> limb)
    {
        switch (limb.Comp.PartType)
        {
            ...
            case BodyPartType.Hand:
                ...
                {
                    if (slotId is null) continue;
                    var slotFullId = SharedBodySystem.GetOrganContainerId(slotId);
                    var child = _containers.GetContainer(limb, slotFullId);
                ...
                if (TryComp<HandsComponent>(body, out var hands))
                    _hands.AddHand((body, hands), SharedBodySystem.GetPartSlotContainerId(slot), limb.Comp.Symmetry == BodyPartSymmetry.Left ? HandLocation.Left : HandLocation.Right);
                break;
        ...
        switch (limb.Comp3.PartType)
        {
            ...
            case BodyPartType.Hand:
                foreach (var organSlotId in limb.Comp3.Organs.Keys)
                {
                    ...
                    var child = _containers.GetContainer(limb, SharedBodySystem.GetOrganContainerId(organSlotId));
                ...
                if (parentSlot is not null && TryComp<HandsComponent>(body, out var hands))
                    _hands.RemoveHand((body, hands), SharedBodySystem.GetPartSlotContainerId(parentSlot.Value.Slot));
                break;
            case BodyPartType.Leg:
            case BodyPartType.Foot:
                break;
